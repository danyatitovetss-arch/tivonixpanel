...
export function ActivityTimeline({ activities, users }: ActivityTimelineProps) {
  ...
  if (sorted.length === 0) {
    return <p className="font-mono text-sm text-[#6b7280]">// история пуста</p>;
  }
          ...
          <div key={activity.id} className="flex gap-4">
            <div className="flex w-5 shrink-0 flex-col items-center">
              <div className="relative mt-2 flex size-5 items-center justify-center">
                <div className="absolute size-5 rounded-full border border-[#050505]/10" />
                <div className="size-2.5 rounded-full bg-[#050505]" />
              </div>
              {!isLast && (
                <div className="my-1 w-px flex-1 min-h-10 bg-[#050505]/15" aria-hidden />
              )}
            </div>
            ...
            <div className={cn("min-w-0 flex-1", isLast ? "pb-0" : "pb-6")}>
              <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                ...
                <span className="font-mono text-xs text-[#050505]/15">—</span>
                <p className="text-sm font-medium leading-snug text-[#050505]">{message}</p>
              </div>
              <p className="mt-1.5 font-mono text-xs text-[#6b7280]">
