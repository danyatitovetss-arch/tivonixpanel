...
export function DailyProgress({ prospects }: { prospects: ProspectContact[] }) {
  ...
  return (
    <div className="rounded-2xl bg-[#f6f6f6] p-5 md:p-6">
      <h3 className="text-sm font-semibold text-[#050505]">Сегодняшняя цель</h3>
          ...
          return (
            <div key={item.label}>
              <div className="flex justify-between text-xs">
                <span className="text-[#050505]">{item.label}</span>
                <span className="text-[#6b7280]">{item.current}/{item.goal}</span>
              </div>
              <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-white">
                <div className="h-full rounded-full bg-[#050505]" style={{ width: `${pct}%` }} />
              </div>
            </div>
          );
        })}
