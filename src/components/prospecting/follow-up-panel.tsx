...
export function FollowUpPanel({ followUpAt, followUpSent, onSchedule }: FollowUpPanelProps) {
  ...
  return (
    <div className="rounded-2xl bg-[#f6f6f6] p-5">
      <h3 className="text-sm font-semibold text-[#050505]">Повторное сообщение</h3>
      <p className="mt-1 text-xs text-[#6b7280]">
        Не спамь. Если после повторного сообщения ответа нет — поставь «Не подходит».
      </p>
      {followUpAt && (
        <p className="mt-2 text-sm text-[#050505]">Запланировано: {followUpAt}</p>
      )}
      {followUpSent && (
      ...
      <div className="mt-3 flex flex-wrap items-end gap-2">
        ...
        <button
          ...
          className="h-10 rounded-full bg-[#050505] px-4 text-sm font-medium text-white"
        >
          Запланировать повтор
        </button>
      </div>
      <div className="mt-4 rounded-xl bg-white p-4">
        <p className="text-sm text-[#6b7280]">{FOLLOW_UP_TEMPLATE}</p>
