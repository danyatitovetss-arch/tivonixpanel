  ...
  if (domains.every((domain) => failed[domain])) {
    return (
      <div className="flex size-14 shrink-0 items-center justify-center rounded-2xl bg-white">
        <Globe className="size-6 text-[#6b7280]" />
      </div>
    );
  }
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-3">
                <h3 className="min-w-0 text-xl font-semibold leading-tight text-[#050505] md:text-2xl">
                  {platform.name}
                </h3>
                <a
                  href={platform.url}
                  ...
                  className={cn(
                    actionButtonClass,
                    "h-10 w-auto shrink-0 bg-[#050505] px-4 text-sm text-white hover:bg-[#262626]"
                  )}
                >
                  Открыть
              ...
              <p className="mt-3 min-h-[4.5rem] text-base leading-relaxed text-[#6b7280] md:text-[17px]">
                {platform.purpose}
              </p>
            </div>
          </div>
        ...
        <div className="flex flex-1 flex-col">
          {platform.whoToLookFor && (
            <SectionBlock title="Кого искать" className="px-5 md:px-6">
              <p className="text-base leading-relaxed text-[#6b7280] md:text-[17px]">{platform.whoToLookFor}</p>
            </SectionBlock>
          )}
          ...
          <SectionBlock title="Что вводить в поиск" className="px-5 md:px-6">
            ...
            >
              <Search className="size-5 shrink-0 text-[#050505]" />
              <span className="min-w-0 flex-1">
                <span className="block text-base font-semibold text-[#050505] md:text-lg">
                  Выбрать запрос ({platform.searchQueries.length})
                </span>
                <span className="mt-1 block text-sm leading-relaxed text-[#6b7280] md:text-base">
                  {preview}
            ...
            <SectionBlock title="Что делать" className="px-5 md:px-6">
              <ol className="space-y-2.5">
                {platform.steps.map((step, i) => (
                  <li key={step} className="flex gap-3 text-base leading-relaxed text-[#6b7280] md:text-[17px]">
                    <span className="flex size-7 shrink-0 items-center justify-center rounded-full bg-white text-sm font-semibold text-[#050505]">
                      {i + 1}
                    </span>
          ...
          <SectionBlock title="Что проверить" className="flex-1 px-5 md:px-6">
            <ul className="space-y-2">
              {platform.whatToCheck.map((item) => (
                <li key={item} className="flex gap-2 text-base leading-relaxed text-[#6b7280] md:text-[17px]">
          ...
          {platform.note && (
            <div className="px-5 py-4 md:px-6">
              <p className="rounded-2xl bg-white px-4 py-3 text-base leading-relaxed text-[#6b7280] md:text-[17px]">
                {platform.note}
              </p>
            </div>
          )}
        ...
        <footer className="mt-auto shrink-0 px-5 pb-5 pt-2 md:px-6">
          <div className="flex flex-col gap-2">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className={cn(actionButtonClass, "bg-[#050505] text-white hover:bg-[#262626]")}
            >
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
